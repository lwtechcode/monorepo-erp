...
export default function SalesMadePage() {
  ...
  const [filterSales, setFilterSales] = useState<FilterSaleBudgetsType>({
  ...
  const { columns, data } = useDataTable({ sales: salesMade?.salesBudgets });

  return (
    <Flex className="h-full" justify="center" vertical>
      <Typography.Title level={4}>Orçamentos</Typography.Title>
