...
async function create(request, response) {
    ...
    try {
        const data = await supplier_service_1.default.create(Object.assign(Object.assign({}, body), { company_id: request.company_id }));
        return response
...
async function findAll(request, response) {
    const { page = 1, pageSize = 10, nameOrCNPJ = '', status = '', orderBy = '', } = request.query;
    const company_id = request.company_id;
    try {
        const data = await supplier_service_1.default.findAll({
            page: Number(page),
...
async function update(request, response) {
    ...
    try {
        const supplier = await supplier_service_1.default.findById({
            id,
            companyId: company_id,
        });
        if (!supplier) {
        ...
        const dataEdited = await supplier_service_1.default.update(id, body, company_id);
...
async function findAllOptions(request, response) {
    const company_id = request.company_id;
    try {
        const data = await supplier_service_1.default.findAllOptions(company_id);
        return response.status(status_code_1.STATUS_CODE.OK).json(data);
    }
    catch (error) {
        console.log(error);
...
async function generatePDF(request, response) {
    try {
        ...
        const company_id = request.company_id;
        const suppliers = await supplier_service_1.default.findAllToReport({
            nameOrCnpj: String(nameOrCNPJ),
            companyId: company_id,
            status: String(status),
            orderBy: String(orderBy),
        });
        const pdfDoc = await supplier_service_1.default.generatePDF(suppliers.suppliers);
        const chunks = [];
