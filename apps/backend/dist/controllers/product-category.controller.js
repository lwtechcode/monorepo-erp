...
async function create(request, response) {
    ...
    try {
        const newData = await product_category_service_1.default.create(Object.assign(Object.assign({}, body), { company_id }));
...
async function findAll(request, response) {
    ...
    const company_id = request.company_id;
    try {
        const data = await product_category_service_1.default.findAll({
            page: Number(page),
...
async function update(request, response) {
    ...
    try {
        const data = await product_category_service_1.default.findById({
            id,
            companyId,
        });
        if (!data) {
        ...
        const dataEdited = await product_category_service_1.default.update({
            companyId,
            data: body,
            paymentId: id,
        });
...
async function findAllOptions(request, response) {
    const company_id = request.company_id;
    try {
        const data = await product_category_service_1.default.findAllOptions(company_id);
        return response.status(status_code_1.STATUS_CODE.OK).json(data);
    }
    catch (error) {
        console.log(error);
