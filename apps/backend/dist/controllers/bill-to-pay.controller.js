...
const bill_to_pay_service_1 = __importDefault(require("../services/bill-to-pay.service"));
...
function generateStatus(due_date, pay_date) {
    ...
    if ((pay_date && (0, date_fns_1.isEqual)(dueDateOnly, pay_date)) ||
        ...
        return status_1.paymentStatusEnum.PAGA;
    }
    ...
    if (pay_date && (0, date_fns_1.isAfter)(pay_date, due_date)) {
        return status_1.paymentStatusEnum.PAGA_COM_ATRASO;
    }
...
async function create(request, response) {
    ...
    try {
        const data = await bill_to_pay_service_1.default.create(Object.assign(Object.assign({}, body), { companyId: request.company_id, status: generateStatus(body.due_date, body.pay_date) }));
        return response
...
async function findAll(request, response) {
    ...
    try {
        const data = await bill_to_pay_service_1.default.findAllPaginated({
            page: Number(page),
...
async function update(request, response) {
    ...
    try {
        const bill = await bill_to_pay_service_1.default.findById({ id, companyId });
        if (!bill) {
            return response
        ...
        const billToPay = await bill_to_pay_service_1.default.update({
            billToPayId: id,
...
async function remove(request, response) {
    ...
    const companyId = request.company_id;
    try {
        const billToPay = await bill_to_pay_service_1.default.findById({ id, companyId });
        if (!billToPay) {
            return response
                .status(status_code_1.STATUS_CODE.NOT_FOUND)
                .json({ message: response_messages_1.RESPONSE_MESSAGE.NOT_FOUND });
        }
        await bill_to_pay_service_1.default.remove({
            id,
            companyId,
        });
        return response
...
async function changeStatus(_, response) {
    try {
        const billsToPay = await bill_to_pay_service_1.default.findAll({
        ...
        if (ids.length > 0) {
            await bill_to_pay_service_1.default.updateStatus({ ids });
        }
...
async function generatePDF(request, response) {
    try {
        ...
        const data = await bill_to_pay_service_1.default.findAllToReport({
        ...
        const pdfDoc = await bill_to_pay_service_1.default.generatePDF(data.billsToPay);
        const chunks = [];
...
exports.default = { create, findAll, update, remove, changeStatus, generatePDF };
